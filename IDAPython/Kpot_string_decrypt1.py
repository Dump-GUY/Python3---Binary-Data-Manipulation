...
#string decrypting func NAME = "Decrypt_strings" - 0040C8F5
string_decrypt_addr = idc.get_name_ea_simple("Decrypt_strings")
print("Func Decrypt_Strings address: 0x%x" % (string_decrypt_addr))
Xref_decrypt_funcs = []
for addr in idautils.CodeRefsTo(string_decrypt_addr, 0):
    Xref_decrypt_funcs.append(addr)
   
print("XREF Decrypt_Strings func COUNT: %d" % (len(Xref_decrypt_funcs)))

for addr in Xref_decrypt_funcs:
    prev_instr = idc.prev_head(addr)
    print("XREF Func Decrypt_Strings prev instruction: 0x%x   %s" % (prev_instr,idc.generate_disasm_line(prev_instr, 0)))
